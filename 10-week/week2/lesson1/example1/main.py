...
def main():
    ...
    scaler = MinMaxScaler()
    train_data = scaler.fit_transform(training_data[['bmi', 'ltg', 'y']])

    # fit our model
    model = pytorch_lr_fit(np.array(train_data[:, 0:2]), np.array(train_data[:, 2]), 0.1, 1000)
    
    # read in and pre-process our test data
    test_data = pd.read_csv('../data/test.csv')
    test_data = scaler.transform(test_data[['bmi', 'ltg', 'y']])

    # test input for the model and observation tensors
    test_input = Variable(torch.from_numpy(test_data[:, 0:2]).float())
    ...
    # calculate our RMSE
    rmse = math.sqrt(mean_squared_error(predictions.data.numpy(), test_data[:, 2]))
